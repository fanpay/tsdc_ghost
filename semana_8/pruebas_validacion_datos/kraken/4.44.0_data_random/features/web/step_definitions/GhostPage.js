...
class GhostPage {
    ...
    async login(username, password){
      await this.load('http://localhost:3002/ghost/#/signin');
    ...
    async logout(){
      await this.load('http://localhost:3002/ghost/#/posts');
      await this.wait(2);
    ...
    async create_post(title, content) {
      await this.load('http://localhost:3002/ghost/#/editor/post');
      await this.wait(3);
      ...
      await this.wait(1);
      await this.click(properties.BTN_BLACK_PUBLISH);
      await this.wait(2);
      await this.click(properties.BTN_BLACK_MODAL);
      await this.wait(2);
    }
    ...
    async delete_first_post(title) {
      ...
      await this.wait(2);
      await this.click(properties.SETTINGS_BUTTON_PUBLICATION);
      await this.wait(2);
